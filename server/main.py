...
def gerar_excel(dados, empresa_override=None):
    ...
    tit   = 'PEDIDO PATA NEGRA DISTRIBUIDORA' if emp==2 else 'PEDIDO INDÚSTRIA PATA NEGRA'
...
def parse_torre_barra(pdf_bytes, produtos):
    """Torre Barra — TOTVS com nome em múltiplas linhas e sem coluna Seq separada."""
    filiais = []
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        n_pags = len(pdf.pages)
        for pi in range(0, n_pags, 2):
...
def parse_superprix(pdf_bytes, produtos):
    """Formato texto corrido — uma linha por item."""
    filiais = []
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        txt = '\n'.join(p.extract_text() or '' for p in pdf.pages)
    ...
    for m in reItem.finditer(txt):
        ...
        emb_raw  = m.group(2)  # KG/00001 ou CX/00020
...
def parse_adonai(pdf_bytes, produtos):
    """SuasVendas — tabela limpa, empresa sempre=2 (sem CNPJ fornecedor)."""
    filiais = []
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        txt = '\n'.join(p.extract_text() or '' for p in pdf.pages)
...
def health():
    perfis = {}
    for c in ['dom_atacarejo','atacadao','assai','torre_central','torre_barra','superprix','germans','adonai']:
        if perfil_existe(c):
...
def upload_perfil(cliente):
    ...
    clientes_validos = ['dom_atacarejo','atacadao','assai','torre_central','torre_barra','superprix','germans','adonai']
...
def processar():
    try:
        ...
        nomes_cliente = {
            ...
            'torre_barra':   'Torre Barra',
            'superprix':     'Superprix',
            'germans':       'Germans',
            'adonai':        'Adonai Atacadista',
        }
        if cliente == 'dom_atacarejo':
            ...
            filiais = parse_germans(pdf_bytes, produtos)
        elif cliente == 'adonai':
            filiais = parse_adonai(pdf_bytes, produtos)
        else:

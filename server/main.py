...
def processar():
    try:
        ...
        for fd in filiais:
            ...
            # FIX (01/08/2026): o nome do arquivo usava a filial/razão social
            # extraída do PDF do pedido (ex.: "EMANUEL COMERCIO DE GENEROS
            # ALIMENTICIOS LTDA" para clientes SuasVendas), deixando o nome
            # longo e quebrando a importação em outro sistema. Voltou a usar
            # só o nome curto do cliente (a chave do parser, ex.: 'obom',
            # 'superprix') + timestamp com MICROSSEGUNDOS — suficiente pro
            # operador reconhecer o cliente de cara, e nunca repete mesmo
...
def processar_manual():
    ...
    try:
        ...
        if lat is not None and lng is not None:
            # FIX (01/08/2026): ver comentário equivalente em /processar.
            ts = datetime.datetime.utcnow().strftime('%Y%m%d_%H%M%S_%f')
            rid = f"{cliente}_{ts}"
            salvar_romaneio(rid, {
                'id': rid,
